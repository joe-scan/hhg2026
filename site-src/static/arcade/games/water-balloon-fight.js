...
function gBalloons() {
  const s = { name: 'WATER BALLOON FIGHT', how: ['SOAK ' + PL[0].name + ' BEFORE YOU GET SOAKED.', 'MOVE AROUND YOUR HALF OF THE GARDEN. FIRE TO THROW.', helper() ? 'IF ' + helper().name + ' LOOKS OUT THE DOOR, DON\'T THROW!' : PET ? PET.name + ' BURSTS ANY BALLOON THAT HITS THEM.' : 'DODGE, WEAVE AND THROW!', 'FIRST TO 5 SPLASHES.'], ctl: 'MOVE: ALL DIRECTIONS   THROW: FIRE', pts: [0, 0], done: -1 };
  const Y0 = 58, Y1 = 258, ZONE = [[24, 190], [290, 456]], COLS = ['#22e6ff', '#ff2bd6', '#3dff8b', '#ffd23f'];
  const NULINES = ['LUNCH IN FIVE MINUTES, LADS!', 'WHO\'S SOAKING WHO OUT HERE?', 'MIND THE WINDOWS NOW!', 'I\'M WATCHING YOU TWO!'];
  ...
  s.update = () => {
    ...
    for (let i = 0; i < 2; i++) {
      ...
      if (I.aP && me.cd <= 0 && nu.ph === 'look') { s.pts[i] = Math.max(0, s.pts[i] - 1); me.cd = 40; nu.line = 'I SAW THAT, ' + PL[i].name + '!'; nu.T = Math.max(nu.T, 50); sfx.buzz(); shake = 4; floatText('-1', me.x, me.y - 44, '#fff', 16); continue; }
    ...
    for (const b of balls) {
      ...
      if (you.wet <= 0 && Math.abs(b.x - you.x) < 12 && b.y > you.y - tall && b.y < you.y + 2) {
        ...
        s.pts[b.who]++; floatText('SPLASH!', you.x, you.y - 48, '#fff', 16); cheer(b.who, W / 2, 76, s.pts, ['BULLSEYE!', 'SOAKED!', 'DRENCHED!']);
  ...
  s.draw = () => {
    ...
    if (pause > 0) txt('READY... THROW!', W / 2, 140, 8, '#fff', 'center', COL.hot);
  };
  return s;
}
