...
function gBoss(kind) {
  const BED = kind === 1;
  const s = { name: BED ? 'EL RELOJ DE DORMIR' : 'MONSTRUO DEBERES', boss: true, pts: [0, 0], done: -1, time: 0 };
  const MAXHP = BED ? 200 : 150;
  const WORDS = BED ? ['ZZZ', 'BOSTEZO', '21H', '¡A LA CAMA!', 'PIJAMA', 'DIENTES', 'DORMIR'] : ['7x8', '9x6', '12x12', '6x7', '8x8', 'SUMAS', 'EXAMENES', 'REDACCION', 'LENGUA', 'TABLAS'];
  ...
  function eat(i, f) {
    ...
    if (f.type === 'pasta') { me.rapid = 420; floatText('¡PODER PASTA! TIRO RAPIDO', me.x, me.y - 50, COL.gold); }
    else if (f.type === 'broc') { hearts = Math.min(6, hearts + 1); floatText('¡BROCOLI! +1 VIDA', me.x, me.y - 50, COL.green); }
    else if (f.type === 'stew') { me.power = 480; floatText('¡PODER GUISO! TIRO TRIPLE', me.x, me.y - 50, COL.gold); }
    else { me.power = 480; floatText('¡PODER CURRY! TIROS DE FUEGO', me.x, me.y - 50, '#ff8a1e'); }
  }
  s.update = () => {
    t++; s.time = t;
    if (dead > 0) {
    ...
    if (meter >= 100 && blast <= 0 && t - lastP[0] <= 30 && t - lastP[1] <= 30) { blast = 70; meter = 0; lastP = [-99, -99]; foes.length = 0; sfx.blast(); say('¡Ataque familiar!'); shake = 12; }
    ...
    for (const f of foes) {
      f.x += f.vx; f.y += f.vy;
      for (let i = 0; i < 2; i++) { const me = p[i]; if (me.inv <= 0 && Math.abs(f.x - me.x) < f.w * .4 + 5 && f.y + f.h / 2 > me.y - 28 && f.y - f.h / 2 < me.y) { if (i !== cpu) hearts--; me.inv = 90; shake = 8; sfx.hurt(); f.y = 999; floatText(BED ? 'SLEEPY...' : '¡AY!', me.x, me.y - 46, '#fff'); if (hearts <= 0) { s.done = 0; return; } } }
    }
    ...
    if (BED) { if (dark > 0) dark--; else if (--nextDark <= 0) { dark = 260; nextDark = rint(460, 640); sfx.buzz(); floatText('¡LUCES FUERA!', W / 2, 120, '#fff', 16); } }
    if (petRuns() && !dog && hearts < 6 && --nextDog <= 0) { dog = { x: -20 }; sfx.woof(); }
    if (dog) {
      dog.x += 1.5;
      for (let i = 0; i < 2 && dog; i++) if (Math.abs(p[i].x - dog.x) < 16 && p[i].y > 228) { hearts++; sfx.power(); floatText(PET.name + '! +1 VIDA', dog.x, 214, COL.green); dog = null; nextDog = 480; }
      if (dog && dog.x > W + 20) { dog = null; nextDog = 360; }
    }
    if (hp <= 0) { dead = 1; foes.length = 0; shots.length = 0; dark = 0; mus.mode = 'off'; say(BED ? '¡Hora de dormir cancelada!' : '¡Deberes destruidos!'); }
  };
  function drawFood(f) {
  ...
  s.draw = () => {
    ...
    if (blast > 0) {
      ...
      txt('¡ATAQUE FAMILIAR!', W / 2, 130, 24, cols[t % 4], 'center', true);
    }
    ...
    txt('EQUIPO', 6, 15, 8, COL.dim);
    rect(140, 5, 200, 8, '#2a1a4a'); rect(140, 5, 200 * clamp(hp / MAXHP, 0, 1), 8, hp < MAXHP * .5 ? COL.red : COL.green); txt(s.name, W / 2, 16, 8, '#fff', 'center');
    rect(W - 86, 5, 80, 8, '#2a1a4a'); rect(W - 86, 5, .8 * meter, 8, meter >= 100 ? (t % 10 < 5 ? '#fff' : COL.gold) : COL.cyan); txt('ATAQUE', W - 6, 16, 8, COL.cyan, 'right');
    if (meter >= 100 && blast <= 0 && dead === 0) { rect(0, 150, W, 16, 'rgba(10,4,22,.7)'); txt(cpu >= 0 ? '¡ATAQUE LISTO! ¡PULSA DISPARO!' : '¡ATAQUE LISTO! ¡PULSAD LOS DOS!', W / 2, 154, 8, t % 20 < 12 ? COL.gold : '#fff', 'center'); }
  };
  return s;
}
