...
function gDinner() {
  const s = { name: 'CARRERA DE LA CENA', how: [(helper() ? helper().name + ' PREPARA LA COMIDA. ' : '') + 'ATRAPALA EN TU PLATO.', 'PASTA Y BROCOLI: 1. TU ' + CFG.food + ': 3.', 'LAS COLES RESTAN 2. ¡PUAJ!', 'PRIMERO A 15.'], ctl: 'MUEVE: IZQUIERDA Y DERECHA', pts: [0, 0], done: -1 };
  const FOOD = {
    pasta: { v: [1, 1], c: '#ffd76b', say: '¡PASTA!' }, broc: { v: [1, 1], c: '#2e9b4a', say: '¡BROCOLI!' },
    ...
    sprout: { v: [-2, -2], c: '#6fae3a', say: '¡PUAJ! -2' }
  };
  const FLOOR = 250, SPEED = 2.6;
  // Sean's plate is a touch wider
  const REACH = [17, 19];
  const NULINES = ['¡LA COMIDA ESTA LISTA!', '¡COMED LA VERDURA!', '¿MANOS LAVADAS?', '¡NO PONGAIS CARAS!', '¡HAY MAS SI QUEREIS!', '¡SENTAOS BIEN LOS DOS!'];
  const cook = helper();
  let p, food, nu = { line: '¡LA COMIDA ESTA LISTA!', T: 150 }, t = 0, next = 30, pause = 40;
    ...
    for (const f of food) {
      ...
      if (f.y > FLOOR - 36 && f.y < FLOOR - 24) {
        ...
        if (d[i] < 1e9) {
          ...
          if (v === 0) { floatText('¡ESE NO ES TUYO!', p[i].x, FLOOR - 60, '#fff'); sfx.blip(); continue; }
          s.pts[i] = Math.max(0, s.pts[i] + v); floatText(FOOD[f.type].say, p[i].x, FLOOR - 60, v > 0 ? PL[i].col : '#fff');
          if (v > 0) { sfx.power(); p[i].yum = 20; burst(f.x, f.y, 10, [FOOD[f.type].c, '#fff'], 2); if (v >= 3) { cheer(i, W / 2, 60, s.pts, ['¡ÑAM!', '¡LA CENA ESTA SERVIDA!', '¡PLATO LIMPIO!']); nu.line = 'MUY BIEN, ' + PL[i].name + '!'; nu.T = 100; } }
          else { sfx.buzz(); shake = 3; nu.line = 'TE SIENTAN BIEN, ' + PL[i].name + '!'; nu.T = 110; }
  ...
  s.draw = () => {
    ...
    if (pause > 0) txt('¡LA CENA ESTA LISTA!', W / 2, 130, 8, '#0a0416', 'center');
  };
  return s;
}
addGame('CARRERA DE LA CENA', gDinner);
