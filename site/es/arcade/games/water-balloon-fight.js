...
function gBalloons() {
  const s = { name: 'GUERRA DE GLOBOS', how: ['MOJA A ' + PL[0].name + ' ANTES DE QUE TE MOJE.', 'MUEVETE POR TU MITAD DEL JARDIN. DISPARO PARA TIRAR.', helper() ? 'SI ' + helper().name + ' MIRA POR LA PUERTA, ¡NO TIRES!' : PET ? PET.name + ' REVIENTA LOS GLOBOS QUE LE DAN.' : '¡ESQUIVA Y TIRA!', 'PRIMERO A 5 REMOJONES.'], ctl: 'MUEVE: TODAS   TIRA: DISPARO', pts: [0, 0], done: -1 };
  const Y0 = 58, Y1 = 258, ZONE = [[24, 190], [290, 456]], COLS = ['#22e6ff', '#ff2bd6', '#3dff8b', '#ffd23f'];
  const NULINES = ['¡COMIDA EN CINCO MINUTOS!', '¿QUIEN MOJA A QUIEN AQUI?', '¡CUIDADO CON LAS VENTANAS!', '¡OS ESTOY VIENDO!'];
  ...
  s.update = () => {
    ...
    if (nu.who && --nu.T <= 0) {
      if (nu.ph === 'away') { nu.ph = 'door'; nu.T = 45; nu.line = 'A VER...'; sfx.blip(); }
    ...
    for (let i = 0; i < 2; i++) {
      ...
      if (I.aP && me.cd <= 0 && nu.ph === 'look') { s.pts[i] = Math.max(0, s.pts[i] - 1); me.cd = 40; nu.line = 'TE HE VISTO, ' + PL[i].name + '!'; nu.T = Math.max(nu.T, 50); sfx.buzz(); shake = 4; floatText('-1', me.x, me.y - 44, '#fff', 16); continue; }
      if (I.aP && me.cd <= 0) { balls.push({ x: me.x + (i === 0 ? 14 : -14), y: me.y - 22, vx: i === 0 ? 4.2 : -4.2, who: i, c: pick(COLS), k: 0 }); me.cd = 28; sfx.throwIt(); }
    }
    for (const b of balls) {
      b.x += b.vx; b.k++;
      if (petRuns() && Math.abs(b.x - 240) < 12 && Math.abs(b.y - (dog.y - 10)) < 12) { b.gone = true; splash(b.x, b.y, 14, b.c); sfx.woof(); dog.bark = 30; floatText(pick([PET.name + ' ¡LO PARA!', 'GRAN PARADA, ' + PET.name + '!', '¡GUAU!']), 240, dog.y - 34, '#fff'); continue; }
      const you = p[1 - b.who];
      ...
      if (you.wet <= 0 && Math.abs(b.x - you.x) < 12 && b.y > you.y - tall && b.y < you.y + 2) {
        ...
        s.pts[b.who]++; floatText('¡CHOF!', you.x, you.y - 48, '#fff', 16); cheer(b.who, W / 2, 76, s.pts, ['¡DIANA!', '¡EMPAPADO!', '¡CALADO!']);
  ...
  s.draw = () => {
    ...
    if (nu.who && nu.ph === 'look') txt(nu.who.name + ' MIRA. ¡NO TIREIS!', W / 2, 244, 8, COL.green, 'center', '#0a0416');
    ...
    if (pause > 0) txt('PREPARADOS... ¡YA!', W / 2, 140, 8, '#fff', 'center', COL.hot);
  };
  return s;
}
addGame('GUERRA DE GLOBOS', gBalloons);
