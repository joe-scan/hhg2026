...
function gSeat() {
  ...
  const watcher = helper() ? helper().name : 'EL CONDUCTOR';
  const s = { name: 'GUERRA EN EL ASIENTO', how: ['APORREA DISPARO PARA EMPUJAR LA LINEA', 'HACIA EL LADO DE ' + PL[0].name + ' EN EL COCHE.', '¡QUIETO CUANDO ' + watcher + ' MIRE POR EL ESPEJO!'], ctl: 'APORREA: DISPARO', pts: [0, 0], done: -1 };
  ...
  s.update = () => {
    ...
    for (let i = 0; i < 2; i++) {
      ...
      if (dad === 'look') { caught[i] = true; line += i === 0 ? -30 : 30; shake = 8; sfx.buzz(); floatText('¡PILLADO!', i === 0 ? 150 : 330, 150, '#fff', 16); }
  ...
  s.draw = () => {
    ...
    txt(PL[0].name + ': SU LADO', 64, 146, 8, PL[0].col); txt(PL[1].name + ': SU LADO', 416, 146, 8, PL[1].col, 'right');
    // mirror
    ...
    if (dad === 'look') { rect(0, 52, W, 18, 'rgba(255,46,77,.9)'); txt(watcher + ' MIRA! ¡QUIETOS!', W / 2, 57, 8, '#fff', 'center'); }
    else if (dad === 'warn') txt('UY UY...', W / 2, 57, 8, '#fff', 'center');
  };
  return s;
}
addGame('GUERRA EN EL ASIENTO', gSeat);
