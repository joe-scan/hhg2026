...
const DEMO = {
  ...
  occasion: 'birthday', catchphrase: '¡NO PUEDE SER!', food: 'PIZZA',
...
const ROLES = {
  dad: { label: 'Papá', adult: 'm', kit: '#2b4a8a', lose: 'TE DEJE GANAR, ¿EH?', food: 'CURRY' },
  mum: { label: 'Mamá', us: 'Mamá', adult: 'f', kit: '#d81b8c', lose: '¿AL MEJOR DE TRES?', food: 'ENSALADA' },
  granny: { label: 'Abuela', us: 'Abuela', adult: 'f', kit: '#2e7d4f', lose: '¡QUE CRACK ERES!', food: 'SOPA', grey: true },
  grandad: { label: 'Abuelo', us: 'Abuelo', adult: 'm', kit: '#7a5a3a', lose: 'EN MIS TIEMPOS...', food: 'GUISO', grey: true },
  auntie: { label: 'Tía', us: 'Tía', adult: 'f', kit: '#7a3cff', lose: '¡OYE, QUE BUENO ERES!', food: 'PASTA' },
  uncle: { label: 'Tío', adult: 'm', kit: '#1e7a34', lose: '¡SUERTE DE PRINCIPIANTE!', food: 'HAMBURGUESA' },
  brother: { label: 'Hermano', kid: 'straight', kit: '#ff6b1a', lose: '¡ESO NO VALE!', food: 'PAPAS FRITAS' },
  sister: { label: 'Hermana', kid: 'ponytail', kit: '#22b573', lose: '¡NO ESTABA LISTO!', food: 'FIDEOS' },
  friend: { label: 'Amigo', kid: 'short', kit: '#00a2b3', lose: '¡REVANCHA MAÑANA!', food: 'PIZZA' },
  bestfriend: { label: 'Mejor amigo', kid: 'curly', kit: '#ffd23f', lose: '¿SEGUIMOS SIENDO AMIGOS?', food: 'HELADO' },
  cousin: { label: 'Primo', kid: 'long', kit: '#b34bd8', lose: '¡ESPERA AL VERANO!', food: 'PERRITOS' },
  teacher: { label: 'Profesor', adult: 'f', kit: '#3a6f5c', lose: '¡TE QUEDAS DESPUES DE CLASE!', food: 'CAFE' },
  coach: { label: 'Entrenador', adult: 'm', kit: '#c2410c', lose: '¡VEINTE VUELTAS, TODOS!', food: 'NARANJAS' }
};
...
const OCCASIONS = { birthday: 'FELIZ CUMPLEAÑOS', christmas: 'FELIZ NAVIDAD', fathers: 'FELIZ DIA DEL PADRE', mothers: 'FELIZ DIA DE LA MADRE', star: 'ERES UNA ESTRELLA' };
...
const LOCKED = ['BATALLA DE PALAS', 'CARRERA DE LA CENA', 'GUERRA EN EL ASIENTO', 'EL CONCURSO FAMILIAR', 'LA PELEA FINAL', 'EL FINAL'];
...
function sanitise(c) {
  c = c || {}; const h = c.hero || {};
  const out = {
    hero: { name: cleanName(h.name, 10).replace(/[!?.,]/g, '') || 'HEROE', hair: oneOf(h.hair, HAIRS, 'short'), hairCol: oneOf(h.hairCol, HAIRCOLS, HAIRCOLS[2]), skin: oneOf(h.skin, SKINS, SKINS[1]), kit: oneOf(h.kit, KITCOLS, KITCOLS[0]) },
...
const MENU = [
  ['1 JUGADOR', 'EL ORDENADOR LLEVA A LA FAMILIA'],
  ['2 JUGADORES', 'UN ADULTO LLEVA A LA FAMILIA'],
  ['MODO FIESTA', 'TODOS JUEGAN UN TURNO CONTRA EL HEROE']
];
...
function applyConfig(c) {
  ...
  SAYNAME[1] = titleCase(h.name); CATCH[1] = CFG.catchphrase || '¡NO PUEDE SER!'; SAYCATCH[1] = titleCase(CATCH[1]);
  setOpponent(0);
}
...
const PETKINDS = { dog: { label: 'Perro', mobile: true }, cat: { label: 'Gato', mobile: true }, rabbit: { label: 'Conejo', mobile: true }, hamster: { label: 'Hámster', mobile: true }, fish: { label: 'Pez', mobile: false } };
...
window.addEventListener('keydown', e => {
  ...
  if (kpos === KONAMI.length) { kpos = 0; bigHead = !bigHead; sfx.power(); floatText(bigHead ? '¡MODO CABEZON!' : 'CABEZAS NORMALES', W / 2, 120, COL.green, 16); }
});
...
function syncButtons() {
  ...
  if (b) { b.setAttribute('aria-pressed', muted ? 'true' : 'false'); b.textContent = muted ? 'Sonido: apagado' : 'Sonido: activado'; }
}
...
const FLAGNAMES = ['PORTUGAL', 'ESPAÑA', 'FRANCIA', 'IRLANDA', 'ITALIA'];
...
const CHEER = {
  nice: ['¡MUY BIEN!', '¡GENIAL!', '¡QUE TIRO!', '¡BRUTAL!', '¡CLAVADO!', '¡ASI SE HACE!'],
  hot: ['¡ESTAS EN RACHA!', '¡IMPARABLE!', '¡NO HAY QUIEN TE PARE!', '¡AL ROJO VIVO!'],
  back: ['¡EMPATE!', '¡REMONTADA!', '¡VUELVES AL PARTIDO!', '¡ESTO SE ANIMA!']
};
let streak = [0, 0];
