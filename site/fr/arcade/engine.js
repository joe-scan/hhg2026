...
const DEMO = {
  ...
  occasion: 'birthday', catchphrase: 'PAS POSSIBLE !', food: 'PIZZA',
...
const ROLES = {
  dad: { label: 'Papa', adult: 'm', kit: '#2b4a8a', lose: 'JE T\'AI LAISSE GAGNER.', food: 'CURRY' },
  mum: { label: 'Maman', us: 'Maman', adult: 'f', kit: '#d81b8c', lose: 'ON FAIT TROIS MANCHES ?', food: 'SALADE' },
  granny: { label: 'Mamie', us: 'Mamie', adult: 'f', kit: '#2e7d4f', lose: 'TU ES TROP FORT !', food: 'SOUPE', grey: true },
  grandad: { label: 'Papi', us: 'Papi', adult: 'm', kit: '#7a5a3a', lose: 'DE MON TEMPS...', food: 'RAGOUT', grey: true },
  auntie: { label: 'Tata', us: 'Tata', adult: 'f', kit: '#7a3cff', lose: 'OH, TU ES DOUE !', food: 'PATES' },
  uncle: { label: 'Tonton', adult: 'm', kit: '#1e7a34', lose: 'CHANCE DE DEBUTANT !', food: 'BURGER' },
  brother: { label: 'Frère', kid: 'straight', kit: '#ff6b1a', lose: 'CA COMPTE PAS !', food: 'FRITES' },
  sister: { label: 'Sœur', kid: 'ponytail', kit: '#22b573', lose: 'J\'ETAIS PAS PRET !', food: 'NOUILLES' },
  friend: { label: 'Ami', kid: 'short', kit: '#00a2b3', lose: 'REVANCHE DEMAIN !', food: 'PIZZA' },
  bestfriend: { label: 'Meilleur ami', kid: 'curly', kit: '#ffd23f', lose: 'TOUJOURS AMIS ?', food: 'GLACE' },
  cousin: { label: 'Cousin', kid: 'long', kit: '#b34bd8', lose: 'ATTENDS CET ETE !', food: 'HOT DOGS' },
  teacher: { label: 'Professeur', adult: 'f', kit: '#3a6f5c', lose: 'TU RESTES APRES LE COURS !', food: 'CAFE' },
  coach: { label: 'Entraîneur', adult: 'm', kit: '#c2410c', lose: 'VINGT TOURS, TOUT LE MONDE !', food: 'ORANGES' }
};
...
const OCCASIONS = { birthday: 'JOYEUX ANNIVERSAIRE', christmas: 'JOYEUX NOEL', fathers: 'BONNE FETE PAPA', mothers: 'BONNE FETE MAMAN', star: 'TU ES UNE STAR' };
...
const LOCKED = ['BATAILLE DE RAQUETTES', 'LA COURSE DU DINER', 'BAGARRE A L\'ARRIERE', 'LE QUIZ DE FAMILLE', 'LE COMBAT DE BOSS', 'LA FIN'];
...
function sanitise(c) {
  c = c || {}; const h = c.hero || {};
  const out = {
    hero: { name: cleanName(h.name, 10).replace(/[!?.,]/g, '') || 'HEROS', hair: oneOf(h.hair, HAIRS, 'short'), hairCol: oneOf(h.hairCol, HAIRCOLS, HAIRCOLS[2]), skin: oneOf(h.skin, SKINS, SKINS[1]), kit: oneOf(h.kit, KITCOLS, KITCOLS[0]) },
...
const MENU = [
  ['1 JOUEUR', 'L\'ORDINATEUR JOUE LA FAMILLE'],
  ['2 JOUEURS', 'UN ADULTE JOUE LA FAMILLE'],
  ['MODE FETE', 'CHACUN JOUE UN TOUR CONTRE LE HEROS']
];
...
function applyConfig(c) {
  ...
  SAYNAME[1] = titleCase(h.name); CATCH[1] = CFG.catchphrase || 'PAS POSSIBLE !'; SAYCATCH[1] = titleCase(CATCH[1]);
  setOpponent(0);
}
...
const PETKINDS = { dog: { label: 'Chien', mobile: true }, cat: { label: 'Chat', mobile: true }, rabbit: { label: 'Lapin', mobile: true }, hamster: { label: 'Hamster', mobile: true }, fish: { label: 'Poisson', mobile: false } };
...
window.addEventListener('keydown', e => {
  ...
  if (kpos === KONAMI.length) { kpos = 0; bigHead = !bigHead; sfx.power(); floatText(bigHead ? 'MODE GROSSE TETE !' : 'TETES NORMALES', W / 2, 120, COL.green, 16); }
});
...
function syncButtons() {
  ...
  if (b) { b.setAttribute('aria-pressed', muted ? 'true' : 'false'); b.textContent = muted ? 'Son : coupé' : 'Son : activé'; }
}
...
const FLAGNAMES = ['PORTUGAL', 'ESPAGNE', 'FRANCE', 'IRLANDE', 'ITALIE'];
...
const CHEER = {
  nice: ['BIEN JOUE !', 'GENIAL !', 'BEAU TIR !', 'TOP !', 'PARFAIT !', 'CONTINUE !'],
  hot: ['TU ES EN FEU !', 'IMPARABLE !', 'EN PLEINE FORME !', 'BRULANT !'],
  back: ['EGALITE !', 'REMONTADA !', 'TU REVIENS !', 'CA COMMENCE !']
};
let streak = [0, 0];
