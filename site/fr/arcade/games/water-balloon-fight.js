...
function gBalloons() {
  const s = { name: 'BATAILLE DE BALLONS D\'EAU', how: ['TREMPE ' + PL[0].name + ' AVANT D\'ETRE TREMPE.', 'BOUGE DANS TA MOITIE DE JARDIN. TIR POUR LANCER.', helper() ? 'SI ' + helper().name + ' REGARDE PAR LA PORTE, NE LANCE PAS !' : PET ? PET.name + ' FAIT ECLATER LES BALLONS QUI LE TOUCHENT.' : 'ESQUIVE ET LANCE !', 'PREMIER A 5 ECLABOUSSURES.'], ctl: 'BOUGE : TOUTES DIRECTIONS   LANCE : TIR', pts: [0, 0], done: -1 };
  const Y0 = 58, Y1 = 258, ZONE = [[24, 190], [290, 456]], COLS = ['#22e6ff', '#ff2bd6', '#3dff8b', '#ffd23f'];
  const NULINES = ['A TABLE DANS CINQ MINUTES !', 'QUI TREMPE QUI ICI ?', 'ATTENTION AUX FENETRES !', 'JE VOUS SURVEILLE !'];
  ...
  s.update = () => {
    ...
    if (nu.who && --nu.T <= 0) {
      if (nu.ph === 'away') { nu.ph = 'door'; nu.T = 45; nu.line = 'BON...'; sfx.blip(); }
    ...
    for (let i = 0; i < 2; i++) {
      ...
      if (I.aP && me.cd <= 0 && nu.ph === 'look') { s.pts[i] = Math.max(0, s.pts[i] - 1); me.cd = 40; nu.line = 'JE T\'AI VU, ' + PL[i].name + '!'; nu.T = Math.max(nu.T, 50); sfx.buzz(); shake = 4; floatText('-1', me.x, me.y - 44, '#fff', 16); continue; }
      if (I.aP && me.cd <= 0) { balls.push({ x: me.x + (i === 0 ? 14 : -14), y: me.y - 22, vx: i === 0 ? 4.2 : -4.2, who: i, c: pick(COLS), k: 0 }); me.cd = 28; sfx.throwIt(); }
    }
    for (const b of balls) {
      b.x += b.vx; b.k++;
      if (petRuns() && Math.abs(b.x - 240) < 12 && Math.abs(b.y - (dog.y - 10)) < 12) { b.gone = true; splash(b.x, b.y, 14, b.c); sfx.woof(); dog.bark = 30; floatText(pick([PET.name + ' L\'ARRETE !', 'BEL ARRET, ' + PET.name + '!', 'OUAF !']), 240, dog.y - 34, '#fff'); continue; }
      const you = p[1 - b.who];
      ...
      if (you.wet <= 0 && Math.abs(b.x - you.x) < 12 && b.y > you.y - tall && b.y < you.y + 2) {
        ...
        s.pts[b.who]++; floatText('SPLASH !', you.x, you.y - 48, '#fff', 16); cheer(b.who, W / 2, 76, s.pts, ['EN PLEIN DANS LE MILLE !', 'TREMPE !', 'DOUCHE !']);
  ...
  s.draw = () => {
    ...
    if (nu.who && nu.ph === 'look') txt(nu.who.name + ' REGARDE. NE LANCEZ PAS !', W / 2, 244, 8, COL.green, 'center', '#0a0416');
    ...
    if (pause > 0) txt('PRET... LANCE !', W / 2, 140, 8, '#fff', 'center', COL.hot);
  };
  return s;
}
addGame('BATAILLE DE BALLONS D\'EAU', gBalloons);
