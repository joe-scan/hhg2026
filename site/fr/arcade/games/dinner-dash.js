...
function gDinner() {
  const s = { name: 'LA COURSE DU DINER', how: [(helper() ? helper().name + ' PREPARE LE REPAS. ' : '') + 'ATTRAPE-LE DANS TON ASSIETTE.', 'PATES, BROCOLI : 1. TES ' + CFG.food + ': 3.', 'LES CHOUX COUTENT 2. BEURK !', 'PREMIER A 15.'], ctl: 'BOUGE : GAUCHE ET DROITE', pts: [0, 0], done: -1 };
  const FOOD = {
    pasta: { v: [1, 1], c: '#ffd76b', say: 'PATES !' }, broc: { v: [1, 1], c: '#2e9b4a', say: 'BROCOLI !' },
    ...
    sprout: { v: [-2, -2], c: '#6fae3a', say: 'BEURK ! -2' }
  };
  const FLOOR = 250, SPEED = 2.6;
  // Sean's plate is a touch wider
  const REACH = [17, 19];
  const NULINES = ['LE REPAS EST PRET !', 'MANGEZ VOS LEGUMES !', 'MAINS LAVEES ?', 'PAS DE GRIMACES !', 'IL Y EN A ENCORE !', 'TENEZ-VOUS DROITS TOUS LES DEUX !'];
  const cook = helper();
  let p, food, nu = { line: 'LE REPAS EST PRET !', T: 150 }, t = 0, next = 30, pause = 40;
    ...
    for (const f of food) {
      ...
      if (f.y > FLOOR - 36 && f.y < FLOOR - 24) {
        ...
        if (d[i] < 1e9) {
          ...
          if (v === 0) { floatText('PAS LE TIEN !', p[i].x, FLOOR - 60, '#fff'); sfx.blip(); continue; }
          s.pts[i] = Math.max(0, s.pts[i] + v); floatText(FOOD[f.type].say, p[i].x, FLOOR - 60, v > 0 ? PL[i].col : '#fff');
          if (v > 0) { sfx.power(); p[i].yum = 20; burst(f.x, f.y, 10, [FOOD[f.type].c, '#fff'], 2); if (v >= 3) { cheer(i, W / 2, 60, s.pts, ['MIAM !', 'LE DINER EST SERVI !', 'ASSIETTE VIDE !']); nu.line = 'BRAVO, ' + PL[i].name + '!'; nu.T = 100; } }
          else { sfx.buzz(); shake = 3; nu.line = 'C\'EST BON POUR TOI, ' + PL[i].name + '!'; nu.T = 110; }
  ...
  s.draw = () => {
    ...
    if (pause > 0) txt('LE DINER EST PRET !', W / 2, 130, 8, '#0a0416', 'center');
  };
  return s;
}
addGame('LA COURSE DU DINER', gDinner);
