...
(() => {
  ...
  const HAIRNAMES = ['Noir', 'Châtain foncé', 'Châtain', 'Châtain clair', 'Auburn', 'Roux', 'Blond', 'Gris'];
  const SKINNAMES = ['Lightest', 'Clair', 'Clair moyen', 'Moyen', 'Moyen foncé', 'Foncé'];
  const KITNAMES = ['Bleu', 'Rouge', 'Vert', 'Jaune', 'Violet', 'Orange', 'Rose', 'Blanc', 'Noir'];
  const PETNAMES = ['Blanc', 'Crème', 'Beige', 'Châtain', 'Noir', 'Gris'];
  ...
  function changed() {
    ...
    $('play-label').textContent = 'Jouez au jeu de ' + titleCase(c.hero.name) + '';
  ...
  function frame() {
    ...
    const occ = OCCASIONS[CFG.occasion] === 'TU ES UNE STAR' ? 'UNE AVENTURE ARCADE' : OCCASIONS[CFG.occasion] + ' EDITION';
