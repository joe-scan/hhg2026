...
const TAUNT = {
  dad: ['MHUIN MISE GACH RUD DUIT.', 'TOG GO BOG E, MO GHLUINE.', 'BHI ME AR FHEABHAS FADO.'],
  mum: ['NI CHAILLIM. FIAFRAIGH DE DO DHAID.', 'NIONN AN CAILLTEOIR NA SOITHI.', 'BHI ME AG CLEACHTADH.'],
  granny: ['TA ME AG IMIRT SEO LE BLIANTA.', 'NA LIG DON CHARDAGAN TU A MHEALLADH.', 'GHEOBHAIDH AN BUAITEOIR BRISCIN.'],
  grandad: ['FADO NI RAIBH ACH CNAIPE AMHAIN.', 'NIL ME ACH AG TEITH SUAS.', 'NI BHUAILEANN AON DUINE DAIDEO.'],
  auntie: ['TA DO ROGHA AINTIN AR AIS!', 'GAN TROCAIRE INNIU.', 'TA ME NIOS FEARR NA D\'UNCAIL.'],
  uncle: ['TABHARFAIDH ME TUS DUIT.', 'FEACH AGUS FOGHLAIM.', 'ABAIR LE DO DHAID GO RAIBH ME AG CUR CEIST AIR.'],
  brother: ['TA TU AG DUL SIOS!', 'NI SHABHALFAIDH DO LA BREITHE FEIN THU.', 'MA DHEANANN TU CAIMILEIREACHT, INSEOIDH ME.'],
  sister: ['REIDH LE CAILLIUINT?', 'IS MISE CURADH AN TI SEO.', 'GAN GHOL NUAIR A CHAILLEANN TU.'],
  friend: ['TA A FHIOS AGAINN BEIRT CE A BHUAFAIDH.', 'GAN DUL SIAR AIR INA DHIAIDH.', 'IOMPRAIONN AN CAILLTEOIR NA MALAI.'],
  bestfriend: ['CAIRDE GO DTI AN FHEADOG.', 'TA A FHIOS AGAM FAOI DO CHLEASA GO LEIR.', 'MHUIN TUSA AN CEANN SEO DOM.'],
  cousin: ['TA ONOIR AN TEAGHLAIGH I mBAOL.', 'TA ME AG FEITHEAMH AR FEADH NA BLIANA.', 'BUAFAIDH MO THAOBH DEN TEAGHLACH.'],
  teacher: ['NIL SE SEO AR AN TRIAIL.', 'TAISPEAIN DO CHUID OIBRE.', 'MARCALAIM GO CRUA.'],
  coach: ['AN NDEARNA TU TEITH SUAS?', 'GAN LEITHSCEALTA AMUIGH ANSIN.', 'CHONAIC ME DO CHUID TRAENALA.']
};
const CHEERS_END = { birthday: 'LÁ BREITHE SONA!', christmas: 'NOLLAIG SHONA!', fathers: 'LÁ NA nAITHREACHA SONA!', mothers: 'LÁ NA MÁITHREACHA SONA!', star: 'MAITH THU!' };
...
function pressFire(y, label) { if (S.t % 50 < 34) txt(label || (touchMode ? 'TAPAIL NO BRUIGH LAMHACH' : 'BRUIGH LAMHACH'), W / 2, y, 8, '#fff', 'center', COL.hot); }
...
const ST = {
  title: {
    ...
    draw() {
      ...
      const occ = CFG.occasion === 'star' ? 'EACHTRA ARCADE' : OCCASIONS[CFG.occasion] + ' EDITION';
      ...
      pressFire(240, touchMode ? 'TAPAIL CHUN TOSU' : 'BRUIGH LAMHACH NO CLICEAIL');
    }
  },
  vs: {
    enter() {
      round++; setOpponent(round - 1); game = GAMES[round - 1].make();
      S.taunt = pick(TAUNT[CHAR[0].role] || ['TOSAIONN SE!']);
      say(SAYNAME[1] + ', in aghaidh, ' + SAYNAME[0] + '!'); mus.mode = 'match'; mus.fast = round === GAMES.length;
    },
  ...
  result: {
    enter() { sfx.score(); whistle(); burst(W / 2, 120, 50, [PL[S.w].col, '#fff'], 5); if (S.w === 1) say(SAYNAME[1] + ' a bhuaigh!'); },
    ...
    draw() {
      rect(0, 0, W, H, '#12062b'); stars(S.t, H); hud('BABHTA ' + round, PL[1].name + ' ' + won[1] + ' - ' + won[0] + ' TEAGHLACH');
  ...
  partySetup: {
    ...
    draw() {
      ...
      txt('MOD COIRME', W / 2, 26, 16, COL.gold, 'center', true);
      txt('IMRIONN GACH DUINE SEAL IN AGHAIDH ' + HERO.name + '.', W / 2, 56, 8, '#fff', 'center');
      txt('CE MHEAD DUSHLANOIR?', W / 2, 92, 8, COL.cyan, 'center');
      for (let k = 2; k <= 6; k++) {
        const on = S.n === k, x = W / 2 + (k - 4) * 56;
        rect(x - 22, 112, 44, 44, on ? COL.hot : 'rgba(255,255,255,.08)');
        txt(String(k), x, 124, 24, on ? '#fff' : COL.dim, 'center');
      }
      txt('SUAS AGUS SIOS CHUN ATHRU', W / 2, 174, 8, COL.dim, 'center');
      txt('IMRIONN GACH DUINE CLUICHE AMHAIN. IMRIONN AN LAOCH GACH CEANN.', W / 2, 196, 8, '#fff', 'center');
  ...
  partyEnd: {
    enter() {
      ...
      say(S.champs.length ? 'Tá curadh againn!' : SAYNAME[1] + ' a bhuaigh ar chách!');
    },
    update() { if (S.t > 90 && (anyIn().aP || clicked)) go('title'); },
    draw() {
      bgSynth(S.t); rect(0, 0, W, H, 'rgba(10,4,22,.86)');
      txt('TORTHAI NA COIRME', W / 2, 18, 16, COL.gold, 'center', true);
      ...
      party.wins.forEach((w, k) => {
        ...
        txt('IMREOIR ' + (k + 1), 122, y + 4, 8, champ ? COL.gold : '#fff');
        txt(w ? 'BHUAIGH AR ' + HERO.name : 'CHAILL LE ' + HERO.name, 360, y + 4, 8, w ? COL.green : COL.dim, 'right');
      });
      const line = S.champs.length === 0 ? HERO.name + ' A BHUAIGH AR CHACH!'
        : S.champs.length === 1 ? 'CURADH: IMREOIR ' + S.champs[0]
        : 'CURAIDH: IMREOIR ' + S.champs.join(' AGUS ');
      txt(line, W / 2, top + rows * gap + 12, 12, COL.cyan, 'center', true);
      boy(1, W / 2, 250, 4, false, Math.floor(S.t / 10));
      if (S.t > 90) pressFire(top + rows * gap + 38, 'BRUIGH LAMHACH DON ROGHCHLAR');
    }
  },

  // the end of the teaser: what they just played, against what the full game has
  locked: {
    enter() { mus.mode = 'title'; mus.fast = false; say('Sin é an taispeántas.'); },
  ...
  bossIntro: {
    enter() {
      ...
      say(S.bed ? 'Ó no. Tá sé in am codlata. ' + SAYNAME[1] + ' agus ' + SAYNAME[0] + ', le chéile!' : 'Arracht na hobairbhaile! ' + SAYNAME[1] + ' agus ' + SAYNAME[0] + ', le chéile!');
    },
    update() { if ((S.t > 120 && (anyIn().aP || clicked)) || S.t > 480) go('boss', { bed: S.bed }); },
    draw() {
      game.draw(); rect(0, 110, W, 142, 'rgba(10,4,22,.88)');
      txt('DUSHLAN AMHAIN EILE. LE CHEILE!', W / 2, 116, 8, COL.red, 'center');
      txt(S.bed ? 'CLOG AM CODLATA' : 'ARRACHT NA hOBAIRBHAILE', W / 2, 132, 16, '#fff', 'center', true);
      txt(S.bed ? (CFG.occasion === 'christmas' ? 'NI THIOCFAIDH DAIDI NA NOLLAG GO DTI GO gCODLAIONN TU!' : 'AR DO LA BREITHE? NI BHEIDH!') : 'TA DO DHEIREADH SEACHTAINE AR FAD UAIDH!', W / 2, 154, 8, COL.gold, 'center');
      txt(PL[1].name + ' AGUS ' + PL[0].name + ' AR AN bhFOIREANN CHEANNA.', W / 2, 170, 8, COL.cyan, 'center');
      txt('COINNIGH LAMHACH CHUN SCAOILEADH. LION AN BARRA,', W / 2, 186, 8, '#fff', 'center');
      txt(two ? 'AGUS TAPAILIGI LAMHACH LE CHEILE!' : 'AGUS TAPAIL LAMHACH DON PHLEASC TEAGHLAIGH!', W / 2, 198, 8, '#fff', 'center');
      txt('FAIGH AN BIA. IS LEATSA NA PLATAI ' + CHAR[1].letter + ' AGUS ' + CHAR[0].letter + '', W / 2, 214, 8, COL.green, 'center');
  ...
  cont: {
    ...
    draw() {
      ...
      txt('CHOMH GAR SIN!', W / 2, 40, 24, '#fff', 'center', true); txt('ARIS?', W / 2, 80, 16, COL.gold, 'center');
      pressFire(226, 'LAMHACH: IARRACHT EILE!');
    }
  },
  // ---- the gift moment
  finale: {
    enter() { mus.mode = 'title'; mus.fast = true; S.lines = FAM.map(m => pick([CHEERS_END[CFG.occasion], 'MAITH THU, ' + HERO.name + '!', 'IS TU AN CEANN IS FEARR!'])); },
    update() {
...
function render() {
  ...
  g.restore();
  if (muted) txt('BALBH', W - 4, H - 10, 8, COL.dim, 'right');
}
function frame(ts) {
...
async function shareGame(btn) {
  const text = S.name === 'finale' ? HERO.name.charAt(0) + HERO.name.slice(1).toLowerCase() + ' a bhuaigh ar an teaghlach ar fad. Tar agus imir an cluiche:' : 'Tar agus imir cluiche arcade ' + titleCase(HERO.name) + ':';
  ...
  try {
    ...
    else { await navigator.clipboard.writeText(text + ' ' + url); btn.textContent = 'Nasc cóipeáilte'; setTimeout(() => { btn.textContent = 'Roinn'; }, 2000); }
  } catch (e) {}
}
