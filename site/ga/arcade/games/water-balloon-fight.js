...
function gBalloons() {
  const s = { name: 'CATH NA mBALUN UISCE', how: ['FLIUCH ' + PL[0].name + ' SULA bhFLIUCHTAR THU.', 'BOG I DO LEATH DEN GHAIRDIN. LAMHACH CHUN CAITHEAMH.', helper() ? 'MA ' + helper().name + ' FHEACHANN AMACH AN DORAS, NA CAITH!' : PET ? PET.name + ' PLEASCANN GACH BALUN A BHUAILEANN E.' : 'SEACHAIN AGUS CAITH!', 'AN CHEAD DUINE GO 5.'], ctl: 'BOG: GACH TREO   CAITH: LAMHACH', pts: [0, 0], done: -1 };
  const Y0 = 58, Y1 = 258, ZONE = [[24, 190], [290, 456]], COLS = ['#22e6ff', '#ff2bd6', '#3dff8b', '#ffd23f'];
  const NULINES = ['LON I gCUIG NOIMEAD!', 'CE ATA AG FLIUCHADH CE?', 'AIRE DO NA FUINNEOGA!', 'TA ME AG FEACHAINT ORAIBH!'];
  ...
  s.update = () => {
    ...
    if (nu.who && --nu.T <= 0) {
      if (nu.ph === 'away') { nu.ph = 'door'; nu.T = 45; nu.line = 'A BHUACHAILLI...'; sfx.blip(); }
    ...
    for (let i = 0; i < 2; i++) {
      ...
      if (I.aP && me.cd <= 0 && nu.ph === 'look') { s.pts[i] = Math.max(0, s.pts[i] - 1); me.cd = 40; nu.line = 'CHONAIC ME E SIN, ' + PL[i].name + '!'; nu.T = Math.max(nu.T, 50); sfx.buzz(); shake = 4; floatText('-1', me.x, me.y - 44, '#fff', 16); continue; }
      if (I.aP && me.cd <= 0) { balls.push({ x: me.x + (i === 0 ? 14 : -14), y: me.y - 22, vx: i === 0 ? 4.2 : -4.2, who: i, c: pick(COLS), k: 0 }); me.cd = 28; sfx.throwIt(); }
    }
    for (const b of balls) {
      b.x += b.vx; b.k++;
      if (petRuns() && Math.abs(b.x - 240) < 12 && Math.abs(b.y - (dog.y - 10)) < 12) { b.gone = true; splash(b.x, b.y, 14, b.c); sfx.woof(); dog.bark = 30; floatText(pick([PET.name + ' A SHABHAIL E!', 'SABHAIL BREA, ' + PET.name + '!', 'AMH!']), 240, dog.y - 34, '#fff'); continue; }
      const you = p[1 - b.who];
      ...
      if (you.wet <= 0 && Math.abs(b.x - you.x) < 12 && b.y > you.y - tall && b.y < you.y + 2) {
        ...
        s.pts[b.who]++; floatText('PLIMP!', you.x, you.y - 48, '#fff', 16); cheer(b.who, W / 2, 76, s.pts, ['DIREACH SA LAR!', 'FLIUCH BAITE!', 'AR MAOS!']);
  ...
  s.draw = () => {
    ...
    if (nu.who && nu.ph === 'look') txt(nu.who.name + ' AG FEACHAINT. NA CAITHIGI!', W / 2, 244, 8, COL.green, 'center', '#0a0416');
    ...
    if (pause > 0) txt('REIDH... CAITH!', W / 2, 140, 8, '#fff', 'center', COL.hot);
  };
  return s;
}
addGame('CATH NA mBALUN UISCE', gBalloons);
