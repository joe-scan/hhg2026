...
function gDinner() {
  const s = { name: 'RAS AN DINNEIR', how: [(helper() ? helper().name + ' AG DEANAMH LOIN. ' : '') + 'BEIR AIR AR DO PHLATA.', 'PASTA, BROCAILI: 1. DO ' + CFG.food + ': 3.', 'BACHLOGA -2. UCH!', 'AN CHEAD DUINE GO 15.'], ctl: 'BOG: CLE AGUS DEAS', pts: [0, 0], done: -1 };
  const FOOD = {
    pasta: { v: [1, 1], c: '#ffd76b', say: 'PASTA!' }, broc: { v: [1, 1], c: '#2e9b4a', say: 'BROCAILI!' },
    ...
    sprout: { v: [-2, -2], c: '#6fae3a', say: 'UCH! -2' }
  };
  const FLOOR = 250, SPEED = 2.6;
  // Sean's plate is a touch wider
  const REACH = [17, 19];
  const NULINES = ['TA AN LON REIDH!', 'ITH DO CHUID GLASRAI!', 'LAMHA NITE?', 'GAN STRAINCEANNA!', 'TA TUILLEADH ANN!', 'SUIGI SUAS DIREACH, AN BHEIRT AGAIBH!'];
  const cook = helper();
  let p, food, nu = { line: 'TA AN LON REIDH!', T: 150 }, t = 0, next = 30, pause = 40;
    ...
    for (const f of food) {
      ...
      if (f.y > FLOOR - 36 && f.y < FLOOR - 24) {
        ...
        if (d[i] < 1e9) {
          ...
          if (v === 0) { floatText('NI LEATSA E!', p[i].x, FLOOR - 60, '#fff'); sfx.blip(); continue; }
          s.pts[i] = Math.max(0, s.pts[i] + v); floatText(FOOD[f.type].say, p[i].x, FLOOR - 60, v > 0 ? PL[i].col : '#fff');
          if (v > 0) { sfx.power(); p[i].yum = 20; burst(f.x, f.y, 10, [FOOD[f.type].c, '#fff'], 2); if (v >= 3) { cheer(i, W / 2, 60, s.pts, ['NEAM!', 'TA AN DINNEAR ULLAMH!', 'PLATA GLAN!']); nu.line = 'MAITH THU, ' + PL[i].name + '!'; nu.T = 100; } }
          else { sfx.buzz(); shake = 3; nu.line = 'TA SIAD GO MAITH DUIT, ' + PL[i].name + '!'; nu.T = 110; }
  ...
  s.draw = () => {
    ...
    if (pause > 0) txt('TA AN DINNEAR REIDH!', W / 2, 130, 8, '#0a0416', 'center');
  };
  return s;
}
addGame('RAS AN DINNEIR', gDinner);
