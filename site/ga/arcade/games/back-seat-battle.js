...
function gSeat() {
  ...
  const watcher = helper() ? helper().name : 'AN TIOMANAI';
  const s = { name: 'CATH AN TSUIOCHAIN CHUIL', how: ['BRUIGH LAMHACH ARIS IS ARIS CHUN AN LINE A BHRU', 'I dTREO THAOBH ' + PL[0].name + ' SA CHARR.', 'FAN SOCAIR NUAIR A ' + watcher + ' FHEACHANN SA SCATHAN!'], ctl: 'BRU ARIS: LAMHACH', pts: [0, 0], done: -1 };
  ...
  s.update = () => {
    ...
    for (let i = 0; i < 2; i++) {
      ...
      if (dad === 'look') { caught[i] = true; line += i === 0 ? -30 : 30; shake = 8; sfx.buzz(); floatText('BEIRTHE!', i === 0 ? 150 : 330, 150, '#fff', 16); }
  ...
  s.draw = () => {
    ...
    txt(PL[0].name + ': A THAOBH', 64, 146, 8, PL[0].col); txt(PL[1].name + ': A THAOBH', 416, 146, 8, PL[1].col, 'right');
    // mirror
    ...
    if (dad === 'look') { rect(0, 52, W, 18, 'rgba(255,46,77,.9)'); txt(watcher + ' AG FEACHAINT! FAN SOCAIR!', W / 2, 57, 8, '#fff', 'center'); }
    else if (dad === 'warn') txt('O HO...', W / 2, 57, 8, '#fff', 'center');
  };
  return s;
}
addGame('CATH AN TSUIOCHAIN CHUIL', gSeat);
