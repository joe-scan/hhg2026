...
function gDinner() {
  const s = { name: 'DINNER DASH', how: [(helper() ? helper().name + '\'S MAKING LUNCH. ' : '') + 'CATCH IT ON YOUR PLATE.', 'PASTA, BROCCOLI: 1. YOUR ' + CFG.food + ': 3.', 'SPROUTS COST YOU 2. YUCK!', 'FIRST TO 15.'], ctl: 'MOVE: LEFT AND RIGHT', pts: [0, 0], done: -1 };
  const FOOD = {
    ...
    sprout: { v: [-2, -2], c: '#6fae3a', say: 'YUCK! -2' }
  };
  const FLOOR = 250, SPEED = 2.6;
  // Sean's plate is a touch wider
  const REACH = [17, 19];
  const NULINES = ['LUNCH IS READY!', 'EAT YOUR GREENS!', 'HANDS WASHED?', 'NO FACES AT THE SPROUTS!', 'THERE\'S MORE IF YOU WANT IT!', 'SIT UP STRAIGHT, THE PAIR OF YOU!'];
  const cook = helper();
  let p, food, nu = { line: 'LUNCH IS READY!', T: 150 }, t = 0, next = 30, pause = 40;
    ...
    for (const f of food) {
      ...
      if (f.y > FLOOR - 36 && f.y < FLOOR - 24) {
        ...
        if (d[i] < 1e9) {
          ...
          if (v === 0) { floatText('NOT YOURS!', p[i].x, FLOOR - 60, '#fff'); sfx.blip(); continue; }
          s.pts[i] = Math.max(0, s.pts[i] + v); floatText(FOOD[f.type].say, p[i].x, FLOOR - 60, v > 0 ? PL[i].col : '#fff');
          if (v > 0) { sfx.power(); p[i].yum = 20; burst(f.x, f.y, 10, [FOOD[f.type].c, '#fff'], 2); if (v >= 3) { cheer(i, W / 2, 60, s.pts, ['YUM!', 'DINNER IS SERVED!', 'CLEAN PLATE!']); nu.line = 'GOOD STUFF, ' + PL[i].name + '!'; nu.T = 100; } }
          else { sfx.buzz(); shake = 3; nu.line = 'THEY\'RE GOOD FOR YOU, ' + PL[i].name + '!'; nu.T = 110; }
  ...
  s.draw = () => {
    ...
    if (pause > 0) txt('DINNER\'S READY!', W / 2, 130, 8, '#0a0416', 'center');
  };
  return s;
}
