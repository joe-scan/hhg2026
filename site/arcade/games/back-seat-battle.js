...
function gSeat() {
  ...
  const watcher = helper() ? helper().name : 'THE DRIVER';
  const s = { name: 'BACK SEAT BATTLE', how: ['MASH FIRE TO PUSH THE LINE', 'ONTO ' + PL[0].name + '\'S SIDE OF THE CAR.', 'FREEZE WHEN ' + watcher + ' LOOKS IN THE MIRROR!'], ctl: 'MASH: FIRE', pts: [0, 0], done: -1 };
  ...
  s.update = () => {
    ...
    for (let i = 0; i < 2; i++) {
      ...
      if (dad === 'look') { caught[i] = true; line += i === 0 ? -30 : 30; shake = 8; sfx.buzz(); floatText('CAUGHT!', i === 0 ? 150 : 330, 150, '#fff', 16); }
  ...
  s.draw = () => {
    ...
    txt(PL[0].name + '\'S SIDE', 64, 146, 8, PL[0].col); txt(PL[1].name + '\'S SIDE', 416, 146, 8, PL[1].col, 'right');
    // mirror
    ...
    if (dad === 'look') { rect(0, 52, W, 18, 'rgba(255,46,77,.9)'); txt(watcher + '\'S LOOKING! FREEZE!', W / 2, 57, 8, '#fff', 'center'); }
    else if (dad === 'warn') txt('UH OH...', W / 2, 57, 8, '#fff', 'center');
  };
  return s;
}
addGame('BACK SEAT BATTLE', gSeat);
