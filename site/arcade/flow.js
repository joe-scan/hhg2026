...
const ST = {
  ...
  vs: {
    ...
    draw() {
      ...
      txt('ROUND ' + round + ' OF ' + GAMES.length, W / 2, 12, 12, COL.dim, 'center');
  ...
  howto: {
    ...
    draw() {
      ...
      if (S.t > 45) pressFire(210, 'PRESS FIRE WHEN READY', 12);
    }
  },
  count: {
    ...
    draw() {
      ...
      if (refName()) { referee(110, 162, 3); txt('REFEREE:', 356, 120, 10, COL.dim, 'center'); txt(refName(), 356, 136, 16, COL.purple, 'center', '#fff'); }
    }
  },
  play: {
    enter() { S.endT = 0; },
  ...
  result: {
    ...
    draw() {
      ...
      txt(PL[S.w].name + ' WINS!', W / 2, 192, 24, PL[S.w].col, 'center', true);
      txt(S.w === 1 ? 'THE FAMILY ARE IN TROUBLE NOW.' : 'NEXT ONE\'S YOURS, ' + PL[1].name + '!', W / 2, 224, 10, '#fff', 'center');
  ...
  partySetup: {
    ...
    draw() {
      ...
      txt('PARTY MODE', W / 2, 26, 16, COL.gold, 'center', true);
      txt('EVERYONE TAKES A TURN AGAINST ' + HERO.name + '.', W / 2, 56, 8, '#fff', 'center');
      txt('HOW MANY CHALLENGERS?', W / 2, 92, 8, COL.cyan, 'center');
      for (let k = 2; k <= 6; k++) {
        const on = S.n === k, x = W / 2 + (k - 4) * 56;
        rect(x - 22, 112, 44, 44, on ? COL.hot : 'rgba(255,255,255,.08)');
        txt(String(k), x, 124, 24, on ? '#fff' : COL.dim, 'center');
      }
      txt('UP AND DOWN TO CHANGE', W / 2, 174, 8, COL.dim, 'center');
      txt('EACH ONE PLAYS ONE GAME. THE HERO PLAYS THEM ALL.', W / 2, 196, 8, '#fff', 'center');
  ...
  partyVs: {
    enter() {
      ...
      say('Player ' + party.i + ', versus, ' + SAYNAME[1] + '!');
    },
    update() { if ((S.t > 60 && (anyIn().aP || clicked)) || S.t > 400) go('howto'); },
    draw() {
      rect(0, 0, W, H, '#12062b'); stars(S.t, H);
      txt('PARTY MODE', W / 2, 12, 8, COL.dim, 'center');
      logo('PLAYER ' + party.i, W / 2, 28, 24, '#ffffff', PL[0].col, mix(PL[0].col, '#000000', .35));
      txt('YOU ARE UP AGAINST ' + HERO.name + '.', W / 2, 64, 8, '#fff', 'center');
      ...
      txt(touchMode ? 'CHALLENGER: THE LEFT PAD' : 'CHALLENGER KEYS: WASD AND SPACE', W / 2, 226, 8, COL.dim, 'center');
  ...
  partyEnd: {
    enter() {
      ...
      say(S.champs.length ? 'We have a champion!' : SAYNAME[1] + ' beat everybody!');
    },
    update() { if (S.t > 90 && (anyIn().aP || clicked)) go('title'); },
    draw() {
      bgSynth(S.t); rect(0, 0, W, H, 'rgba(10,4,22,.86)');
      txt('PARTY RESULTS', W / 2, 18, 16, COL.gold, 'center', true);
      ...
      party.wins.forEach((w, k) => {
        ...
        txt('PLAYER ' + (k + 1), 122, y + 4, 8, champ ? COL.gold : '#fff');
        txt(w ? 'BEAT ' + HERO.name : 'LOST TO ' + HERO.name, 360, y + 4, 8, w ? COL.green : COL.dim, 'right');
      });
      const line = S.champs.length === 0 ? HERO.name + ' BEAT EVERYBODY!'
        : S.champs.length === 1 ? 'CHAMPION: PLAYER ' + S.champs[0]
        : 'CHAMPIONS: PLAYER ' + S.champs.join(' AND ');
      txt(line, W / 2, top + rows * gap + 12, 12, COL.cyan, 'center', true);
      boy(1, W / 2, 250, 4, false, Math.floor(S.t / 10));
      if (S.t > 90) pressFire(top + rows * gap + 38, 'PRESS FIRE FOR THE MENU');
    }
  },
