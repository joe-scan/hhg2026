...
(() => {
  ...
  const HAIRNAMES = ['Nero', 'Castano scuro', 'Castano', 'Castano chiaro', 'Ramato', 'Rosso', 'Biondo', 'Grigio'];
  const SKINNAMES = ['Lightest', 'Chiara', 'Chiara media', 'Media', 'Media scura', 'Scura'];
  const KITNAMES = ['Blu', 'Rosso', 'Verde', 'Giallo', 'Viola', 'Arancione', 'Rosa', 'Bianco', 'Nero'];
  const PETNAMES = ['Bianco', 'Crema', 'Beige', 'Castano', 'Nero', 'Grigio'];
  ...
  function changed() {
    ...
    $('play-label').textContent = 'Gioca al gioco di ' + titleCase(c.hero.name) + '';
  ...
  function frame() {
    ...
    const occ = OCCASIONS[CFG.occasion] === 'SEI UNA STELLA' ? 'UN\'AVVENTURA ARCADE' : OCCASIONS[CFG.occasion] + ' EDITION';
