...
function pressFire(y, label, size) { if (S.t % 50 < 34) txt(label || (touchMode ? 'TOCCA O PREMI FUOCO' : 'PREMI FUOCO'), W / 2, y, size || 8, '#fff', 'center', COL.hot); }
...
const ST = {
  title: {
    ...
    draw() {
      ...
      const occ = CFG.occasion === 'star' ? 'UN\'AVVENTURA ARCADE' : OCCASIONS[CFG.occasion];
      ...
      pressFire(250, touchMode ? 'TOCCA PER INIZIARE' : 'PREMI FUOCO', 12);
    }
  },
  vs: {
    enter() {
    ...
    draw() {
      ...
      txt('ROUND ' + round + ' DI ' + GAMES.length, W / 2, 12, 12, COL.dim, 'center');
  ...
  howto: {
    ...
    draw() {
      ...
      if (S.t > 45) pressFire(210, 'PREMI FUOCO QUANDO SEI PRONTO', 12);
    }
  },
  count: {
  ...
  result: {
    ...
    draw() {
      ...
      txt(PL[S.w].name + ' VINCE!', W / 2, 192, 24, PL[S.w].col, 'center', true);
      txt(S.w === 1 ? 'ORA LA FAMIGLIA E NEI GUAI.' : 'IL PROSSIMO E TUO, ' + PL[1].name + '!', W / 2, 224, 10, '#fff', 'center');
  ...
  partyVs: {
    ...
    draw() {
      ...
      txt(touchMode ? 'SFIDANTE: I TASTI A SINISTRA' : 'TASTI DELLO SFIDANTE: WASD E SPAZIO', W / 2, 226, 8, COL.dim, 'center');
  ...
  partyEnd: {
    enter() {
      const pad = document.getElementById('family-pad');
      if (pad) pad.textContent = 'FAMIGLIA';
  ...
  locked: {
    ...
    draw() {
      ...
      txt('IL GIOCO COMPLETO CONTINUA:', W / 2, 50, 12, '#fff', 'center');
      ...
      txt('E ' + HERO.name + ' VEDE IL SUO NOME A LUCI ALLA FINE.', W / 2, top + rows * gap + 8, 10, COL.cyan, 'center');
      if (S.t > 60) pressFire(top + rows * gap + 32, 'PREMI FUOCO PER RIGIOCARE', 10);
    }
  },
  bossIntro: {
    enter() {
  ...
  finale: {
    ...
    draw() {
      ...
      if (t > 240) txt('HA VINTO ' + won[1] + ' DI ' + GAMES.length + ' GIOCHI. BOSS BATTUTO IN ' + bossTime + 'S.', W / 2, 74, 10, '#fff', 'center', '#0a0416');
