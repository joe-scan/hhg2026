...
const DEMO = {
  ...
  occasion: 'birthday', catchphrase: 'MA DAI!', food: 'PIZZA',
...
const ROLES = {
  dad: { label: 'Papà', adult: 'm', kit: '#2b4a8a', lose: 'TI HO LASCIATO VINCERE.', food: 'CURRY' },
  mum: { label: 'Mamma', us: 'Mamma', adult: 'f', kit: '#d81b8c', lose: 'AL MEGLIO DI TRE?', food: 'INSALATA' },
  granny: { label: 'Nonna', us: 'Nonna', adult: 'f', kit: '#2e7d4f', lose: 'SEI UN CAMPIONE!', food: 'ZUPPA', grey: true },
  grandad: { label: 'Nonno', us: 'Nonno', adult: 'm', kit: '#7a5a3a', lose: 'AI MIEI TEMPI...', food: 'SPEZZATINO', grey: true },
  auntie: { label: 'Zia', us: 'Zia', adult: 'f', kit: '#7a3cff', lose: 'OH, SEI BRAVO!', food: 'PASTA' },
  uncle: { label: 'Zio', adult: 'm', kit: '#1e7a34', lose: 'FORTUNA DEL PRINCIPIANTE!', food: 'HAMBURGER' },
  brother: { label: 'Fratello', kid: 'straight', kit: '#ff6b1a', lose: 'NON VALE!', food: 'PATATINE' },
  sister: { label: 'Sorella', kid: 'ponytail', kit: '#22b573', lose: 'NON ERO PRONTO!', food: 'NOODLES' },
  friend: { label: 'Amico', kid: 'short', kit: '#00a2b3', lose: 'RIVINCITA DOMANI!', food: 'PIZZA' },
  bestfriend: { label: 'Migliore amico', kid: 'curly', kit: '#ffd23f', lose: 'ANCORA AMICI?', food: 'GELATO' },
  cousin: { label: 'Cugino', kid: 'long', kit: '#b34bd8', lose: 'ASPETTA L\'ESTATE!', food: 'HOT DOGS' },
  teacher: { label: 'Insegnante', adult: 'f', kit: '#3a6f5c', lose: 'TI FERMI DOPO LA LEZIONE!', food: 'CAFFE' },
  coach: { label: 'Allenatore', adult: 'm', kit: '#c2410c', lose: 'VENTI GIRI, TUTTI QUANTI!', food: 'ARANCE' }
};
...
const OCCASIONS = { birthday: 'BUON COMPLEANNO', christmas: 'BUON NATALE', fathers: 'BUONA FESTA DEL PAPA', mothers: 'BUONA FESTA DELLA MAMMA', star: 'SEI UNA STELLA' };
...
const LOCKED = ['BATTAGLIA DI RACCHETTE', 'LA CORSA DELLA CENA', 'BATTAGLIA SUL SEDILE', 'IL QUIZ DI FAMIGLIA', 'LA SFIDA AL BOSS', 'IL FINALE'];
...
function sanitise(c) {
  c = c || {}; const h = c.hero || {};
  const out = {
    hero: { name: cleanName(h.name, 10).replace(/[!?.,]/g, '') || 'EROE', hair: oneOf(h.hair, HAIRS, 'short'), hairCol: oneOf(h.hairCol, HAIRCOLS, HAIRCOLS[2]), skin: oneOf(h.skin, SKINS, SKINS[1]), kit: oneOf(h.kit, KITCOLS, KITCOLS[0]) },
...
const MENU = [
  ['1 GIOCATORE', 'IL COMPUTER GIOCA LA FAMIGLIA'],
  ['2 GIOCATORI', 'UN ADULTO GIOCA LA FAMIGLIA'],
  ['MODALITA FESTA', 'TUTTI GIOCANO UN TURNO CONTRO L\'EROE']
];
...
function applyConfig(c) {
  ...
  SAYNAME[1] = titleCase(h.name); CATCH[1] = CFG.catchphrase || 'MA DAI!'; SAYCATCH[1] = titleCase(CATCH[1]);
  setOpponent(0);
}
...
const PETKINDS = { dog: { label: 'Cane', mobile: true }, cat: { label: 'Gatto', mobile: true }, rabbit: { label: 'Coniglio', mobile: true }, hamster: { label: 'Criceto', mobile: true }, fish: { label: 'Pesce', mobile: false } };
...
window.addEventListener('keydown', e => {
  ...
  if (kpos === KONAMI.length) { kpos = 0; bigHead = !bigHead; sfx.power(); floatText(bigHead ? 'MODALITA TESTONE!' : 'TESTE NORMALI', W / 2, 120, COL.green, 16); }
});
...
function syncButtons() {
  ...
  if (b) { b.setAttribute('aria-pressed', muted ? 'true' : 'false'); b.textContent = muted ? 'Audio: spento' : 'Audio: acceso'; }
}
...
const FLAGNAMES = ['PORTUGAL', 'SPAGNA', 'FRANCIA', 'IRLANDA', 'ITALIA'];
...
const CHEER = {
  nice: ['BRAVO!', 'FANTASTICO!', 'CHE TIRO!', 'GRANDE!', 'PERFETTO!', 'COSI SI FA!'],
  hot: ['SEI IN FIAMME!', 'INARRESTABILE!', 'CHE SERIE!', 'INFUOCATO!'],
  back: ['PARI!', 'RIMONTA!', 'SEI TORNATO!', 'SI COMINCIA!']
};
let streak = [0, 0];
