...
function gSeat() {
  ...
  const watcher = helper() ? helper().name : 'CHI GUIDA';
  const s = { name: 'BATTAGLIA SUL SEDILE', how: ['MARTELLA FUOCO PER SPINGERE LA LINEA', 'VERSO IL LATO DI ' + PL[0].name + ' IN MACCHINA.', 'FERMO QUANDO ' + watcher + ' GUARDA NELLO SPECCHIETTO!'], ctl: 'MARTELLA: FUOCO', pts: [0, 0], done: -1 };
  ...
  s.update = () => {
    ...
    for (let i = 0; i < 2; i++) {
      ...
      if (dad === 'look') { caught[i] = true; line += i === 0 ? -30 : 30; shake = 8; sfx.buzz(); floatText('BECCATO!', i === 0 ? 150 : 330, 150, '#fff', 16); }
  ...
  s.draw = () => {
    ...
    txt(PL[0].name + ': IL SUO LATO', 64, 146, 8, PL[0].col); txt(PL[1].name + ': IL SUO LATO', 416, 146, 8, PL[1].col, 'right');
    // mirror
    ...
    if (dad === 'look') { rect(0, 52, W, 18, 'rgba(255,46,77,.9)'); txt(watcher + ' GUARDA! FERMI!', W / 2, 57, 8, '#fff', 'center'); }
    else if (dad === 'warn') txt('UH OH...', W / 2, 57, 8, '#fff', 'center');
  };
  return s;
}
addGame('BATTAGLIA SUL SEDILE', gSeat);
