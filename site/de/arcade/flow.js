...
const TAUNT = {
  dad: ['ICH HAB DIR ALLES BEIGEBRACHT.', 'SEI NETT, MEINE KNIE.', 'FRUEHER WAR ICH RICHTIG GUT.'],
  mum: ['ICH VERLIERE NIE. FRAG PAPA.', 'WER VERLIERT, SPUELT AB.', 'ICH HABE GEUEBT.'],
  granny: ['ICH SPIELE DAS SEIT JAHREN.', 'LASS DICH VON DER STRICKJACKE NICHT TAEUSCHEN.', 'DER SIEGER KRIEGT EINEN KEKS.'],
  grandad: ['ZU MEINER ZEIT GAB ES EINEN KNOPF.', 'ICH WAERME MICH ERST AUF.', 'KEINER SCHLAEGT OPA.'],
  auntie: ['DEINE LIEBLINGSTANTE IST DA!', 'HEUTE KEINE GNADE.', 'ICH BIN BESSER ALS DEIN ONKEL.'],
  uncle: ['DU KRIEGST VORSPRUNG.', 'SCHAU UND LERNE.', 'GRUESS DEINEN PAPA VON MIR.'],
  brother: ['DU GEHST UNTER!', 'NICHT MAL DEIN GEBURTSTAG RETTET DICH.', 'WENN DU SCHUMMELST, PETZE ICH.'],
  sister: ['BEREIT ZU VERLIEREN?', 'ICH BIN DER CHAMPION HIER.', 'NICHT WEINEN, WENN DU VERLIERST.'],
  friend: ['WIR WISSEN BEIDE, WER GEWINNT.', 'HINTERHER GILT KEIN RUECKZIEHER.', 'WER VERLIERT, TRAEGT DIE TASCHEN.'],
  bestfriend: ['BESTE FREUNDE BIS ZUM PFIFF.', 'ICH KENNE ALL DEINE TRICKS.', 'DAS HAST DU MIR BEIGEBRACHT.'],
  cousin: ['ES GEHT UM DIE FAMILIENEHRE.', 'ICH WARTE SEIT EINEM JAHR.', 'MEINE SEITE DER FAMILIE GEWINNT.'],
  teacher: ['DAS KOMMT NICHT IN DER ARBEIT DRAN.', 'RECHENWEG ZEIGEN.', 'ICH BENOTE STRENG.'],
  coach: ['HAST DU DICH AUFGEWAERMT?', 'DA DRAUSSEN GIBT ES KEINE AUSREDEN.', 'ICH HABE DEIN TRAINING GESEHEN.']
};
...
const ST = {
  ...
  vs: {
    enter() {
      ...
      S.taunt = pick(TAUNT[CHAR[0].role] || ['JETZT GEHT ES LOS!']);
      say(SAYNAME[1] + ', gegen, ' + SAYNAME[0] + '!'); mus.mode = 'match'; mus.fast = round === GAMES.length;
    },
    ...
    draw() {
      ...
      txt('RUNDE ' + round + ' VON ' + GAMES.length, W / 2, 12, 12, COL.dim, 'center');
  ...
  howto: {
    ...
    draw() {
      game.draw(); rect(0, 0, W, H, 'rgba(10,4,22,.86)'); hud('RUNDE ' + round, '');
      ...
      txt(touchMode ? 'TOUCH: PFEILE BEWEGEN, GROSSE TASTE FEUERT' : two ? PL[0].name + ': WASD + LEERTASTE    ' + PL[1].name + ': PFEILE + ENTER' : 'PFEILE ODER WASD.  FEUER: LEERTASTE ODER ENTER', W / 2, 176, 8, COL.dim, 'center');
      if (S.t > 45) pressFire(210, 'FEUER DRUECKEN, WENN BEREIT', 12);
    }
  },
  count: {
    enter() { say('Runde ' + round + '. Go!'); },
    update() { if (S.t % 40 === 1 && S.t < 120) sfx.count(); if (S.t === 121) whistle(); if (S.t > 150) go('play'); },
    draw() {
      game.draw(); hud(game.name, game.sub ? game.sub() : '');
      const n = S.t < 120 ? String(3 - Math.floor(S.t / 40)) : 'LOS!';
      rect(0, 106, W, 58, 'rgba(10,4,22,.72)'); txt(n, W / 2, 120, 32, S.t < 120 ? '#fff' : COL.green, 'center', true);
      if (refName()) { referee(110, 162, 3); txt('SCHIEDSRICHTER:', 356, 120, 10, COL.dim, 'center'); txt(refName(), 356, 136, 16, COL.purple, 'center', '#fff'); }
    }
  },
  play: {
    enter() { S.endT = 0; },
  ...
  result: {
    ...
    draw() {
      ...
      txt(PL[S.w].name + ' GEWINNT!', W / 2, 192, 24, PL[S.w].col, 'center', true);
      txt(S.w === 1 ? 'JETZT WIRD ES ENG FUER DIE FAMILIE.' : 'DIE NAECHSTE GEHOERT DIR, ' + PL[1].name + '!', W / 2, 224, 10, '#fff', 'center');
  ...
  partyVs: {
    ...
    draw() {
      ...
      txt(touchMode ? 'HERAUSFORDERER: DAS LINKE FELD' : 'TASTEN DES HERAUSFORDERERS: WASD UND LEERTASTE', W / 2, 226, 8, COL.dim, 'center');
  ...
  partyEnd: {
    enter() {
      const pad = document.getElementById('family-pad');
      if (pad) pad.textContent = 'FAMILIE';
  ...
  locked: {
    ...
    draw() {
      bgSynth(S.t); rect(0, 0, W, H, 'rgba(10,4,22,.82)');
      txt('DAS WAR DIE DEMO', W / 2, 26, 16, COL.gold, 'center', true);
      txt('DAS VOLLE SPIEL GEHT WEITER:', W / 2, 50, 12, '#fff', 'center');
      ...
      txt('UND ' + HERO.name + ' SIEHT AM ENDE DEN NAMEN IN LICHTERN.', W / 2, top + rows * gap + 8, 10, COL.cyan, 'center');
      if (S.t > 60) pressFire(top + rows * gap + 32, 'FEUER DRUECKEN FUER NOCH EINE RUNDE', 10);
    }
  },
  bossIntro: {
    enter() {
      ...
      say(S.bed ? 'Oh nein. Es ist Schlafenszeit. ' + SAYNAME[1] + ' und ' + SAYNAME[0] + ', zusammen!' : 'Das Hausaufgabenmonster! ' + SAYNAME[1] + ' und ' + SAYNAME[0] + ', zusammen!');
    },
    update() { if ((S.t > 120 && (anyIn().aP || clicked)) || S.t > 480) go('boss', { bed: S.bed }); },
    draw() {
      game.draw(); rect(0, 110, W, 142, 'rgba(10,4,22,.88)');
      txt('EINE LETZTE PRUEFUNG. ZUSAMMEN!', W / 2, 116, 8, COL.red, 'center');
      txt(S.bed ? 'DIE SCHLAFENSZEIT-UHR' : 'DAS HAUSAUFGABENMONSTER', W / 2, 132, 16, '#fff', 'center', true);
      txt(S.bed ? (CFG.occasion === 'christmas' ? 'DER WEIHNACHTSMANN KOMMT ERST, WENN DU SCHLAEFST!' : 'AN DEINEM GEBURTSTAG? NIEMALS!') : 'ES WILL DEIN GANZES WOCHENENDE!', W / 2, 154, 8, COL.gold, 'center');
      txt(PL[1].name + ' UND ' + PL[0].name + ' IM SELBEN TEAM.', W / 2, 170, 8, COL.cyan, 'center');
      txt('FEUER HALTEN ZUM SCHIESSEN. LADE DIE LEISTE,', W / 2, 186, 8, '#fff', 'center');
      txt(two ? 'DANN BEIDE GLEICHZEITIG FEUERN!' : 'DANN FEUER FUER DEN FAMILIENSCHUSS!', W / 2, 198, 8, '#fff', 'center');
      txt('SCHNAPP DIR DAS ESSEN. DIE ' + CHAR[1].letter + ' UND ' + CHAR[0].letter + ' TELLER GEHOEREN DIR.', W / 2, 214, 8, COL.green, 'center');
  ...
  cont: {
    ...
    draw() {
      ...
      txt('SO KNAPP!', W / 2, 40, 24, '#fff', 'center', true); txt('NOCHMAL?', W / 2, 80, 16, COL.gold, 'center');
      pressFire(226, 'FEUER: NOCH EIN VERSUCH!');
    }
  },
  // ---- the gift moment
  finale: {
    enter() { mus.mode = 'title'; mus.fast = true; S.lines = FAM.map(m => pick([CHEERS_END[CFG.occasion], 'GUT GEMACHT, ' + HERO.name + '!', 'DU BIST DIE BESTE!'])); },
    update() {
    ...
    draw() {
      ...
      if (t > 240) txt('GEWANN ' + won[1] + ' VON ' + GAMES.length + ' SPIELE. ENDGEGNER IN ' + bossTime + 'S.', W / 2, 74, 10, '#fff', 'center', '#0a0416');
      if (t > 360) pressFire(258, 'TEILEN DRUECKEN UND AN DIE FAMILIE SCHICKEN');
    }
  }
};
...
function render() {
  ...
  g.restore();
  if (muted) txt('STUMM', W - 4, H - 10, 8, COL.dim, 'right');
}
function frame(ts) {
...
async function shareGame(btn) {
  const text = S.name === 'finale' ? HERO.name.charAt(0) + HERO.name.slice(1).toLowerCase() + ' hat die ganze Familie geschlagen. Spiel das Spiel:' : 'Spiel das Arcade-Spiel von ' + titleCase(HERO.name) + ':';
  ...
  try {
    ...
    else { await navigator.clipboard.writeText(text + ' ' + url); btn.textContent = 'Link kopiert'; setTimeout(() => { btn.textContent = 'Teilen'; }, 2000); }
  } catch (e) {}
}
