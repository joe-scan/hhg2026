...
const ST = {
  title: {
    ...
    draw() {
      ...
      pressFire(226, touchMode ? 'TIPPEN ZUM START' : 'FEUER DRUECKEN ODER KLICKEN');
    }
  },
  vs: {
    enter() {
    ...
    draw() {
      ...
      txt('RUNDE ' + round + ' VON ' + GAMES.length, W / 2, 14, 8, COL.dim, 'center');
  ...
  howto: {
    ...
    draw() {
      ...
      if (S.t > 45) pressFire(212, 'FEUER DRUECKEN, WENN BEREIT');
    }
  },
  count: {
    ...
    draw() {
      ...
      if (refName()) { referee(110, 162, 3); txt('SCHIEDSRICHTER:', 356, 122, 8, COL.dim, 'center'); txt(refName(), 356, 136, 16, COL.purple, 'center', '#fff'); }
    }
  },
  play: {
    enter() { S.endT = 0; },
  ...
  result: {
    ...
    draw() {
      ...
      txt(PL[S.w].name + ' GEWINNT!', W / 2, 196, 16, PL[S.w].col, 'center', true);
      txt(S.w === 1 ? 'JETZT WIRD ES ENG FUER DIE FAMILIE.' : 'DIE NAECHSTE GEHOERT DIR, ' + PL[1].name + '!', W / 2, 220, 8, '#fff', 'center');
  ...
  locked: {
    ...
    draw() {
      ...
      txt('DAS VOLLE SPIEL GEHT WEITER:', W / 2, 54, 8, '#fff', 'center');
      ...
      txt('UND ' + HERO.name + ' SIEHT AM ENDE DEN NAMEN IN LICHTERN.', W / 2, top + rows * gap + 8, 8, COL.cyan, 'center');
      if (S.t > 60) pressFire(top + rows * gap + 30, 'FEUER DRUECKEN FUER NOCH EINE RUNDE');
    }
  },
  bossIntro: {
    enter() {
  ...
  finale: {
    ...
    draw() {
      ...
      if (t > 240) txt('GEWANN ' + won[1] + ' VON ' + GAMES.length + ' SPIELE. ENDGEGNER IN ' + bossTime + 'S.', W / 2, 74, 8, '#fff', 'center', '#0a0416');
