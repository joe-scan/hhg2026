...
function gDinner() {
  const s = { name: 'RENNEN ZUM ESSEN', how: [(helper() ? helper().name + ' MACHT ESSEN. ' : '') + 'FANG ES MIT DEINEM TELLER.', 'NUDELN, BROKKOLI: 1. DEIN ' + CFG.food + ': 3.', 'ROSENKOHL KOSTET 2. IGITT!', 'WER ZUERST 15 HAT.'], ctl: 'BEWEGEN: LINKS UND RECHTS', pts: [0, 0], done: -1 };
  const FOOD = {
    pasta: { v: [1, 1], c: '#ffd76b', say: 'NUDELN!' }, broc: { v: [1, 1], c: '#2e9b4a', say: 'BROKKOLI!' },
    ...
    sprout: { v: [-2, -2], c: '#6fae3a', say: 'IGITT! -2' }
  };
  const FLOOR = 250, SPEED = 2.6;
  // Sean's plate is a touch wider
  const REACH = [17, 19];
  const NULINES = ['ESSEN IST FERTIG!', 'ESST EUER GEMUESE!', 'HAENDE GEWASCHEN?', 'KEINE GRIMASSEN!', 'ES IST GENUG DA!', 'GERADE SITZEN, IHR ZWEI!'];
  const cook = helper();
  let p, food, nu = { line: 'ESSEN IST FERTIG!', T: 150 }, t = 0, next = 30, pause = 40;
    ...
    for (const f of food) {
      ...
      if (f.y > FLOOR - 36 && f.y < FLOOR - 24) {
        ...
        if (d[i] < 1e9) {
          ...
          if (v === 0) { floatText('NICHT DEINER!', p[i].x, FLOOR - 60, '#fff'); sfx.blip(); continue; }
          s.pts[i] = Math.max(0, s.pts[i] + v); floatText(FOOD[f.type].say, p[i].x, FLOOR - 60, v > 0 ? PL[i].col : '#fff');
          if (v > 0) { sfx.power(); p[i].yum = 20; burst(f.x, f.y, 10, [FOOD[f.type].c, '#fff'], 2); if (v >= 3) { cheer(i, W / 2, 60, s.pts, ['LECKER!', 'DAS ESSEN IST SERVIERT!', 'TELLER LEER!']); nu.line = 'GUT GEMACHT, ' + PL[i].name + '!'; nu.T = 100; } }
          else { sfx.buzz(); shake = 3; nu.line = 'DAS IST GESUND, ' + PL[i].name + '!'; nu.T = 110; }
  ...
  s.draw = () => {
    ...
    if (pause > 0) txt('ESSEN IST FERTIG!', W / 2, 130, 8, '#0a0416', 'center');
  };
  return s;
}
addGame('RENNEN ZUM ESSEN', gDinner);
