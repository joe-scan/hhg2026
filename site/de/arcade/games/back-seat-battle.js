...
function gSeat() {
  ...
  const watcher = helper() ? helper().name : 'DER FAHRER';
  const s = { name: 'RÜCKBANK-SCHLACHT', how: ['FEUER HAEMMERN UND DIE LINIE SCHIEBEN', 'AUF DIE SEITE VON ' + PL[0].name + ' IM AUTO.', 'STILLHALTEN, WENN ' + watcher + ' IN DEN SPIEGEL SCHAUT!'], ctl: 'HAEMMERN: FEUER', pts: [0, 0], done: -1 };
  ...
  s.update = () => {
    ...
    for (let i = 0; i < 2; i++) {
      ...
      if (dad === 'look') { caught[i] = true; line += i === 0 ? -30 : 30; shake = 8; sfx.buzz(); floatText('ERWISCHT!', i === 0 ? 150 : 330, 150, '#fff', 16); }
  ...
  s.draw = () => {
    ...
    txt(PL[0].name + ': SEINE SEITE', 64, 146, 8, PL[0].col); txt(PL[1].name + ': SEINE SEITE', 416, 146, 8, PL[1].col, 'right');
    // mirror
    ...
    if (dad === 'look') { rect(0, 52, W, 18, 'rgba(255,46,77,.9)'); txt(watcher + ' SCHAUT! STILLHALTEN!', W / 2, 57, 8, '#fff', 'center'); }
    else if (dad === 'warn') txt('OH OH...', W / 2, 57, 8, '#fff', 'center');
  };
  return s;
}
addGame('RÜCKBANK-SCHLACHT', gSeat);
