...
function gPaddle() {
  const s = { name: 'PADDEL-SCHLACHT', how: ['EIN BALL, ZWEI PADDEL.', 'BLOCKEN, DANN MIT DER KANTE ZIELEN.', 'WER ZUERST 3 HAT, GEWINNT.'], ctl: 'BEWEGEN: HOCH UND RUNTER', pts: [0, 0], done: -1 };
  ...
  function point(i) {
    s.pts[i]++; shake = 6; sfx.score(); floatText(PL[i].name + ' TRIFFT!', W / 2, 120, PL[i].col, 16); cheer(i, W / 2, 142, s.pts);
...
addGame('PADDEL-SCHLACHT', gPaddle);
