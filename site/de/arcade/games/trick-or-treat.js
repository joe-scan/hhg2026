...
function gTrickOrTreat(seconds) {
  const s = { name: 'SUESSES ODER SAURES', score: 0, missed: 0, over: false };
    ...
    for (const d of drops) {
      ...
      if (!d.got && d.y > FLOOR - 16 && d.y < FLOOR + 6 && Math.abs(d.x - x) < 22) {
        ...
        if (d.sprout) {
          ...
          sfx.buzz(); shake = 4; floatText('IGITT! -2', d.x, FLOOR - 44, '#3dff8b');
        } else {
          s.score++; sfx.blip();
  ...
  s.draw = () => {
    ...
    txt('SUESSIGKEITEN: ' + s.score, W / 2, 6, 12, '#fff', 'center');
