...
const DEMO = {
  ...
  occasion: 'birthday', catchphrase: 'NIE IM LEBEN!', food: 'PIZZA',
...
const ROLES = {
  dad: { label: 'Papa', adult: 'm', kit: '#2b4a8a', lose: 'ICH HAB DICH GEWINNEN LASSEN.', food: 'CURRY' },
  mum: { label: 'Mama', us: 'Mama', adult: 'f', kit: '#d81b8c', lose: 'BEST OF THREE?', food: 'SALAT' },
  granny: { label: 'Oma', us: 'Oma', adult: 'f', kit: '#2e7d4f', lose: 'DU BIST JA SPITZE!', food: 'SUPPE', grey: true },
  grandad: { label: 'Opa', us: 'Opa', adult: 'm', kit: '#7a5a3a', lose: 'ZU MEINER ZEIT...', food: 'EINTOPF', grey: true },
  auntie: { label: 'Tante', us: 'Tante', adult: 'f', kit: '#7a3cff', lose: 'OH, DU BIST GUT!', food: 'NUDELN' },
  uncle: { label: 'Onkel', adult: 'm', kit: '#1e7a34', lose: 'ANFAENGERGLUECK!', food: 'BURGER' },
  brother: { label: 'Bruder', kid: 'straight', kit: '#ff6b1a', lose: 'DAS ZAEHLT NICHT!', food: 'POMMES' },
  sister: { label: 'Schwester', kid: 'ponytail', kit: '#22b573', lose: 'ICH WAR NICHT BEREIT!', food: 'ASIA-NUDELN' },
  friend: { label: 'Freund', kid: 'short', kit: '#00a2b3', lose: 'REVANCHE MORGEN!', food: 'PIZZA' },
  bestfriend: { label: 'Bester Freund', kid: 'curly', kit: '#ffd23f', lose: 'NOCH BESTE FREUNDE?', food: 'EIS' },
  cousin: { label: 'Cousin', kid: 'long', kit: '#b34bd8', lose: 'WARTE BIS ZUM SOMMER!', food: 'HOT DOGS' },
  teacher: { label: 'Lehrer', adult: 'f', kit: '#3a6f5c', lose: 'NACHSITZEN FUER DICH!', food: 'KAFFEE' },
  coach: { label: 'Trainer', adult: 'm', kit: '#c2410c', lose: 'ZWANZIG RUNDEN, ALLE!', food: 'ORANGEN' }
};
...
const OCCASIONS = { birthday: 'ALLES GUTE', christmas: 'FROHE WEIHNACHTEN', fathers: 'SCHOENEN VATERTAG', mothers: 'SCHOENEN MUTTERTAG', star: 'DU BIST EIN STAR' };
...
const LOCKED = ['PADDEL-SCHLACHT', 'RENNEN ZUM ESSEN', 'RÜCKBANK-SCHLACHT', 'DAS FAMILIENQUIZ', 'DER ENDGEGNER', 'DAS ENDE'];
...
function sanitise(c) {
  c = c || {}; const h = c.hero || {};
  const out = {
    hero: { name: cleanName(h.name, 10).replace(/[!?.,]/g, '') || 'HELD', hair: oneOf(h.hair, HAIRS, 'short'), hairCol: oneOf(h.hairCol, HAIRCOLS, HAIRCOLS[2]), skin: oneOf(h.skin, SKINS, SKINS[1]), kit: oneOf(h.kit, KITCOLS, KITCOLS[0]) },
...
function applyConfig(c) {
  ...
  SAYNAME[1] = titleCase(h.name); CATCH[1] = CFG.catchphrase || 'NIE IM LEBEN!'; SAYCATCH[1] = titleCase(CATCH[1]);
  setOpponent(0);
}
...
const PETKINDS = { dog: { label: 'Hund', mobile: true }, cat: { label: 'Katze', mobile: true }, rabbit: { label: 'Kaninchen', mobile: true }, hamster: { label: 'Hamster', mobile: true }, fish: { label: 'Fisch', mobile: false } };
...
window.addEventListener('keydown', e => {
  ...
  if (kpos === KONAMI.length) { kpos = 0; bigHead = !bigHead; sfx.power(); floatText(bigHead ? 'GROSSKOPF-MODUS!' : 'NORMALE KOEPFE', W / 2, 120, COL.green, 16); }
});
...
function syncButtons() {
  ...
  if (b) { b.setAttribute('aria-pressed', muted ? 'true' : 'false'); b.textContent = muted ? 'Ton: aus' : 'Ton: an'; }
}
...
const FLAGNAMES = ['PORTUGAL', 'SPANIEN', 'FRANKREICH', 'IRLAND', 'ITALIEN'];
...
const CHEER = {
  nice: ['STARK!', 'SUPER!', 'TOLLER WURF!', 'KLASSE!', 'SITZT!', 'WEITER SO!'],
  hot: ['DU BRENNST!', 'NICHT ZU STOPPEN!', 'DU LAEUFST HEISS!', 'GLUEHEND HEISS!'],
  back: ['AUSGLEICH!', 'COMEBACK!', 'WIEDER DRIN!', 'JETZT GEHT ES LOS!']
};
let streak = [0, 0];
