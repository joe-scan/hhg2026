...
(() => {
  ...
  const HAIRNAMES = ['Schwarz', 'Dunkelbraun', 'Braun', 'Hellbraun', 'Rotbraun', 'Rot', 'Blond', 'Grau'];
  const SKINNAMES = ['Lightest', 'Hell', 'Hell mittel', 'Mittel', 'Mittel dunkel', 'Dunkel'];
  const KITNAMES = ['Blau', 'Rot', 'Grün', 'Gelb', 'Lila', 'Orange', 'Pink', 'Weiß', 'Schwarz'];
  const PETNAMES = ['Weiß', 'Creme', 'Beige', 'Braun', 'Schwarz', 'Grau'];
  ...
  function changed() {
    ...
    $('play-label').textContent = 'Spiele ' + titleCase(c.hero.name) + 's Spiel';
  ...
  function frame() {
    ...
    const occ = OCCASIONS[CFG.occasion] === 'DU BIST EIN STAR' ? 'EIN ARCADE-ABENTEUER' : OCCASIONS[CFG.occasion] + ' EDITION';
